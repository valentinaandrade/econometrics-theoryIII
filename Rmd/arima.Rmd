...
modelos_arma <- function(serie, ar_orders = 1:6, ma_orders = 1:6){
  ...
  modelos <- crossing(ar = ar_orders, ma = ma_orders) |> 
    pmap_df(function(ar = 6, ma = 3){
      ...
      ajuste <- Arima(serie, order = c(ar, 0, ma), include.mean = TRUE)
...
modelo <- Arima(serie,  order = c(4, 0, 0), include.mean = TRUE)
modelo
```
...
```{r}
knitr::kable(modelo_summary(modelo))
```
...
autoplot_acf_pact(modelo$residuals) +
  plot_annotation(title = str_glue("Modelo ARMA({AR}, {MA}) {nombre_serie}: ACF y PACF"))
```
